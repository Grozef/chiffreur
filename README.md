...
2. Choisir un mode (onglet) :
   ...
   - **🗝️ Clés RSA** — pour le partage. On chiffre avec la clé **publique** (diffusable),
...
> ⚠️ Un navigateur **ne peut pas modifier ni supprimer** un fichier du disque. L'outil
...
| Rôle | Algorithme | Détail |
|---|---|---|
| Chiffrement du contenu | **AES-256-GCM** | Chiffrement *authentifié* : chiffre **et** détecte toute altération (pas de HMAC séparé). |
| Dérivation depuis un mot de passe | **PBKDF2-HMAC-SHA256** | 310 000 itérations (recommandation OWASP) pour ralentir le bruteforce. |
| Mode clés / partage | **RSA-4096 OAEP-SHA256** | Emballe une clé de session AES aléatoire (chiffrement hybride, modèle PGP). |
| Aléa (clés, IV, sels) | **`crypto.getRandomValues`** | Générateur cryptographiquement sûr. |
...
Ce projet est né du TP **« Chiffrement de fichiers sensibles »** (cf. `../consignes .md`),
...
courants. L'objectif a dépassé le simple rendu du TP : produire un **outil de chiffrement
universel** réellement utilisable.

> Une variante équivalente en **PowerShell / .NET** (native Windows) avait aussi été écrite
> pour le TP ; elle utilisait AES-256-CBC + HMAC (faute d'AES-GCM sous .NET Framework). Ce
> dossier ne conserve que l'outil web, qui la remplace avantageusement (AES-GCM, portable,
> multi-plateforme).
